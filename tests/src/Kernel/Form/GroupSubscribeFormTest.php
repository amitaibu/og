...
class GroupSubscribeFormTest extends KernelTestBase {
  ...
  protected function setUp() {
    ...
    $role = OgRole::loadByGroupTypeGroupBundleAndName('node', $groupBundle1, OgRoleInterface::ANONYMOUS);
    $role
      ->grantPermission('subscribe')
      ->save();

    // Change the permissions of group to allow "subscribe without approval".
    $role = OgRole::loadByGroupTypeGroupBundleAndName('node', $groupBundle2, OgRoleInterface::ANONYMOUS);
    $role
    ...
    $role = OgRole::loadByGroupTypeGroupBundleAndName('node', $groupBundle3, OgRoleInterface::ANONYMOUS);
    $role
      ->grantPermission('subscribe without approval')
      ->save();
